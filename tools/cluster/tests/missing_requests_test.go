...
func TestMissingRequests(t *testing.T) {
	...
	// reqTx, err := chClient.DepositFunds(2 * iotaclient.DefaultGasBudget)
	chEnv.DepositFunds(iotaclient.DefaultGasBudget, userWallet)
	// require.NoError(t, err)
	// _, err = chain.CommitteeMultiClient().WaitUntilAllRequestsProcessedSuccessfully(context.Background(), chainID, reqTx, true, 30*time.Second)
	// require.NoError(t, err)
