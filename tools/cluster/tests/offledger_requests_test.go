...
func TestOffledgerRequestAccessNode(t *testing.T) { // pass
...
func testOffledgerRequest(t *testing.T, e *ChainEnv) {
	...
	chClient := chainclient.New(e.Clu.L1Client(), e.Clu.WaspClient(0), e.Chain.ChainID, e.Clu.Config.ISCPackageID(), e.Chain.OriginatorKeyPair)
	_, err := chClient.DepositFunds(10 * isc.Million)
	require.NoError(t, err)
	balance1, err := chClient.L1Client.GetBalance(context.TODO(), iotaclient.GetBalanceRequest{Owner: chClient.KeyPair.Address().AsIotaAddress()})
	require.NoError(t, err)
	_, err = chClient.PostOffLedgerRequest(context.Background(),
